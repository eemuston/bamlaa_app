...
const HandleSuggestion = () => {
  ...
  const deleteSuggestionMutation = useMutation({
    ...
    onSuccess: (_, word)=> {
        ...
        dispatch('CUSTOM', {message: `Ehdotus: ${word.word} poistettu ehdotuksista!`, color: 'green'})
    }
  })
  ...
  return (
    <div>
        ...
        <div>
            ...
            <button onClick={() => newWordMutation.mutate(suggestions[0])}>Hyväksy</button>
            <button onClick={() => refuseSuggestion(suggestions[0])}>Hylkää</button>
        </div>
        :
